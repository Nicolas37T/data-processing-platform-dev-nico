...
def delete_tmp_dirs(context):
    """
    Function to delete temporary directories created during the DAG execution.
    This function is also used as a callback for successful or failed DAG runs.
    """    
    ...
    # Call to delete XComs after cleaning up temporary files
...
def create_dag(dag,connection_id,id_dag=None):
    ...
    def _get_conversion_data(**context):
        """
        Task function to fetch conversion data from the PostgreSQL database.
        This function pulls necessary data, converts paths, and pushes values to XComs.
        """
        ti = context["ti"]
        ...
        # Fetch and process results, store them in XComs
        results = []
        ...
        # Push important data points to XCom for use in other tasks
        ...
        # Push various report details and paths to XComs
        ...
        # Determine if downloaded files are directory or single file, push to XCom
    ...
    def _read_file(**context):
        """
        Task function to read downloaded files, checking their integrity.
        If all files are corrupted, notifies the next task. Otherwise, proceeds to extraction.
        """
        ti = context['ti']
        dag_run = context['dag_run']
        ...
        # Check each file, set status based on integrity
        ...
        # If all files are corrupted, move to the notification task; otherwise, proceed with extraction
        if all(item.get('status') is not None for item in downloaded_files):
            ...
            print('Next task notify_corrupt_file')
            return 'notify_corrupt_file'                
        else:
            ti.xcom_push(key='read_files', value=downloaded_files)
            print("Next task extraction")
            return 'extraction'

    def _extraction(**context):
        """
        Task function to extract data from files. If extraction fails, status is updated.
        The task branches based on the result of the extraction process.
        """
        ti = context['ti']
        dag_run = context['dag_run']
        code = (ti.xcom_pull(key='code', task_ids='get_conversion_data'))
        ...
        # Attempt extraction for each file, updating status accordingly
        ...
        # Decide next task based on extraction success
        if all(item.get('status') is not None for item in read_files):
            ...
            ti.xcom_push(key='corrupted_files_path', value=corrupted_files_path) 
            print('Next task notify_extraction_error')
            return 'notify_extraction_error'     
        else:
            ti.xcom_push(key='extracted_files', value=read_files)
            print("Next task structure_review")
            return 'structure_review'
        
    def _structure_review(**context):
        """
        Task function to review the structure of extracted files. Checks against a template to ensure correct structure.
        If any structure change is detected, the process is halted and a notification is triggered.
        """
        ti = context['ti']
        dag_run = context['dag_run']
        code = (ti.xcom_pull(key='code', task_ids='get_conversion_data'))
        ...
        # Review each extracted file, checking against template structure
        ...
        # Decide next task based on structure review success
        ...
        if not any(item.get('status') == "successful_conversion" for item in extracted_files):
            print('Next task notify_structure_change')
            return 'notify_structure_change'     
        else:
            ti.xcom_push(key='reviewed_files', value=extracted_files) 
            print("Next task report_data_validation")
            return 'report_data_validation'    
        
    def _report_data_validation(ti):
        """
        Task function that performs the conversion of reviewed files.
        
        It selects the first file marked as successfully reviewed, retrieves relevant
        metadata from XComs, and performs a conversion using the Executor class.
        The converted file path is then converted to a Unix format and pushed to XCom.
        
        If the conversion is successful, it moves to the 'record_conversion' task.
        """
        mongo_hook = MongoHook(conn_id="mongo_db") 
        client = mongo_hook.get_conn()  
        # Retrieve the list of reviewed files from XCom    
        ...
        # Execute the conversion using the Executor class 
        ...
        # If the conversion is successful, push the converted file to XCom and proceed to 'record_conversion'
        if converted_file:
            ti.xcom_push(key='converted_file', value=converted_file)
            print("Next task record_conversion")
            return 'record_conversion'
    
    def _trigger_dag(**context):
        ti = context['ti']
        ...
        dag_bag = DagBag()
        if migration_dag in dag_bag.dags:
            TriggerDagRunOperator(
                task_id=f'trigger_{code}',
                trigger_dag_id=migration_dag,
    ...
    with dag:
        ...
        # notify_success_conversion = PostgresOperator(
        #     task_id="notify_success_conversion",
        #     postgres_conn_id=connection_id,
        #     sql="sql/insert_conversion_status.sql",
        #     params= {'task':"structure_review", 'key':"corrupted_files_path"},
        # )
    ...
    report_data_validation >> record_conversion >> update_report >>trigger_task#>> notify_success_conversion