...
def create_dag(dag,connection_id,id_dag=None):
    ...
    def _check_new_data(ti):
        ...
        files_dicts = executor.check_new_data(url, str(updatedTo), file_path, key_words, format='%Y-%m-%d')#investgar el tipo de dato que se guarda en los xcoms
        ti.xcom_push(key='files_dicts', value = files_dicts)
        if not files_dicts:
            print("Entro aqui record_revision",files_dicts)
            return 'record_revision_only' #record_revision_pre
        else:
            print("Entro aqui store files ",files_dicts)
            return 'store_files' #store_files
