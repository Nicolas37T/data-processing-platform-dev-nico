...
def create_dag(dag,connection_id,id_dag=None,ALL=False):
    ...
    def log_dag_run_info(context):
        ...
        if get_state('notify_url_broken'):
            ...
            files = ti.xcom_pull(task_ids='store_new_data', key='files_dicts')
        else:
    ...
    def _read_download_data(ti):
        ...
        last_file_path = convert_win_path((ti.xcom_pull(key='return_value', task_ids='get_download_data'))[0][7])
        short_name = (ti.xcom_pull(key='return_value', task_ids='get_download_data'))[0][8]
    ...
    def _get_data(ti):
        ...
        file_updated_to = (ti.xcom_pull(key='file_updated_to', task_ids='read_download_data'))
        
        if inspect.iscoroutinefunction(executor.get_data):
            temp_path = asyncio.run(executor.get_data(url, file_path,updated_to=file_updated_to, format='%Y-%m-%d'))
        else: 
            temp_path = executor.get_data(url, file_path,updated_to=file_updated_to, format='%Y-%m-%d')
        ti.xcom_push(key='temp_path', value=temp_path)
        if not temp_path:
            print("Entro aqui record_revision",temp_path)
            return 'record_revision_only' #record_revision_pre
        else:
            print("Entro aqui store files ",temp_path)
            return 'store_new_data' #store_files
        
    def _store_new_data(ti):
        """ Renombra y guarda el archivo en una carpeta con la fecha correspondiente 
        Args:
            filePath(str): path del archivo 
            file(str): path del archivo que se quiere almacenar
            name file(str): nombre con el que se renombrara el archivo
        Returns:
            {key:[]}({}): retorna un diccionario de tipo: 
            { '2022-07-17': ['/home/datax-ubuntu/spim/2022-07-17/2022-07-17_publicacion.xlsx', 
                            '/home/datax-ubuntu/spim/2022-07-17/2022-07-17_publicacion.pdf'],
                            ....
            }
        """
        path = (ti.xcom_pull(key='file_path', task_ids='read_download_data'))
        last_file_path =(ti.xcom_pull(key='last_file_path', task_ids='read_download_data'))
        tmp_path = (ti.xcom_pull(key='temp_path', task_ids='get_data'))
        files_dicts = executor.store_new_data(path, tmp_path,last_file_path,ALL=ALL)

        if not files_dicts:
            return 'record_download_revision' #record_revision_post
        else:
            print("VALOR DE files_dicts", files_dicts)
            ...
            return 'record_download' #store_files
    ...
    with dag:
        ...
        notify_success_revision_only = PostgresOperator(
            ...
            params= {'state':"'Successful Revision'"}
        )

        store_new_data = BranchPythonOperator(
            task_id='store_new_data',
            python_callable=_store_new_data,
        )
        ...
        record_download =PostgresOperator(
            ...
            postgres_conn_id=connection_id,
            sql="sql/insert_download_multi.sql",
    ...
    get_download_data >> read_download_data >> verify_url >> [get_data, notify_url_broken]
    get_data >> [record_revision_only, store_new_data]
    record_revision_only >> notify_success_revision_only
    store_new_data >> [record_download_revision, record_download]
    record_download >> update_file >> notify_success_download >> assigner >> trigger_task
