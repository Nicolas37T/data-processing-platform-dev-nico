...
def create_dag(dag,connection_id,id_dag=None):
    ...
    def _get_file_url(ti):
        """
        Obtiene los URLs del último archivo de la Web.

        Args:
            ti (TaskInstance): Instancia de Tarea.

        Returns:
            list: Lista con los URLs del archivo (formato: xlsx, pdf, ...).
        """
        files_urls = []
        url = (ti.xcom_pull(key='file_url', task_ids='read_download_data'))
        type = (ti.xcom_pull(key='file_type', task_ids='read_download_data'))
        updatedTo = (ti.xcom_pull(key='file_updated_to', task_ids='read_download_data'))
        files_urls = executor.get_file_url(url, str(updatedTo))#investgar el tipo de dato que se guarda en los xcoms
        ti.xcom_push(key='files_urls', value=files_urls)
        url
    ...
    def _file_download(ti):
        """
        Descarga el archivo dentro del directorio especificado en la carpeta temporal (si no existe, la crea).
        ...
        """
        file_paths = []
        files_urls = ti.xcom_pull(key='files_urls', task_ids='get_file_url')
        file_path = (ti.xcom_pull(key='file_path', task_ids='read_download_data'))
        file_paths = executor.file_download(files_urls, file_path)
    ...
    with dag:
        ...
        get_file_url = BranchPythonOperator(
            task_id='get_file_url',
            python_callable=_get_file_url,
        )
        
        file_download = PythonOperator(
            task_id='file_download',
            python_callable=_file_download,
        )
        ...
        download_revision = PostgresOperator(
            task_id="download_revision",
            postgres_conn_id=connection_id,
            sql="sql/insert_review.sql",
    ...
    get_download_data >> read_download_data >> verify_url >> [get_file_url, notify_url_broken]
    notify_url_broken
    get_file_url >> [review_only, file_download]
    review_only >> notify_success_revision_only
    file_download >> compare_files >> [store_files, download_revision] 
    download_revision >> notify_success_download_revision
