...
class D_BO_000000076(CNDC):

    def get_file_url(self, main_url, updated_to,key_words="temperatura central" , format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            date = [re.search(re_date, str(value)) for index,value in enumerate(sub_set) if re.search(re_date, str(value)) and  pd.notna(df_new_file[df_new_file.columns[2]].iloc[index])]
            ...
            if len(date):                
                ...
                year = [re.search(re_year, str(line)) for line in sub_set if re.search(re_year, str(line))]
                month = [re.search(re_month, str(value),re.IGNORECASE) for index,value in enumerate(sub_set) if re.search(re_month, str(value),re.IGNORECASE) and  pd.notna(df_new_file[df_new_file.columns[1]].iloc[index])]
                
                year = int(year[-1].group())
                month = month[-1].group()
...
#     robot = D_BO_000000076()
#     # x = robot.get_file_url(main_url="https://www.cndc.bo/estadisticas/anual.php",updated_to="2023-11-21")
#     # print(x)
#     y = robot.compare_files(files_paths=[{"tmp_path": r"C:\Users\Kevin Padilla\Downloads\temperaturact_2024.zip"}],updated_to="2023-01-21")
#     print(y)

Executor_D_BO_000000076 = D_BO_000000076
Robot = D_BO_000000076
