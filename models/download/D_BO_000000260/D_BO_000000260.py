...
class D_BO_000000260(Download_Base):
    ...
    def get_file_url(self, main_url, updated_to,format='%Y-%m-%d'):
        ...
        try:
            ...
            # Iterate over each year and month from updated_to year to the current year
            for year in range(updated_to.year,today.year+1):
                for month in months:
                    num_month = month_to_number(month=month)
                    day = last_day_month(year=year, month=num_month)
                    date_formated = format_date(f"{year}-{num_month}-{day}")
                    
                    if date_formated<=updated_to:
                        continue
                    # Construct URL for the given year
                    url = f"https://www.aps.gob.bo/?option=com_ajax&module=apsfilesdisplayer&method=getFiles&format=json&folder=webdocs___UNE___DP___Estadisticas___Prestaciones+y+Beneficios___Prestaciones+de+Vejez___{year}___{month}&format=json"
                    ...
                    if not len(files):
                        continue
                    file = [f for f in files if re.search(r"sexo|genero",f["filename"], re.IGNORECASE)][0]
...
#     robot = Executor_D_BO_000000260()
    # x = robot.get_file_url(main_url="http://senamhi.gob.bo/index.php/pcpn",updated_to="2023-12-31")
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"C:\Users\Kevin Padilla\Downloads\Reporte_Jubilacion_AFP_SEXO_202410.pdf"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-06-30")
    # print(y)

Executor_D_BO_000000260 = D_BO_000000260
Robot = D_BO_000000260
