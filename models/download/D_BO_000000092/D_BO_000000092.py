...
class D_BO_000000092(BCB_Boletin_Mensual):

    def get_file_url(self, main_url, updated_to, key_words = "tasas interbancarias",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            if df_new_file.iloc[:, 0].isna().mean() > 0.5:
                df_new_file = df_new_file.iloc[:,1:]
            ...
            months = [re.search(re_month, str(value),re.IGNORECASE) for index,value in enumerate(sub_set) if re.search(re_month, str(value),re.IGNORECASE) and not df_new_file.iloc[index,1:].empty]
...
#     robot = Executor_D_BO_000000092()
#     # x = robot.get_file_url(main_url="https://www.bcb.gob.bo/?q=pub_boletin-mensual",updated_to="2023-10-21")
#     # print(len(x))
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"\\10.0.0.9\spim\BCB\02-Indicadores Economicos\06-Tasas de Interés y Tipos de Cambio\05-Otras tablas de Tasas\02-Interbancarias\2024\2024-05-31_tasas interbancarias.xlsx"}]
    # y = robot.compare_files(files_paths=files,updated_to="2023-01-21")
    # print(y)

Executor_D_BO_000000092 = D_BO_000000092
Robot = D_BO_000000092
