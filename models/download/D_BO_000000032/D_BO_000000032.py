...
import pandas as pd
from models.download.tools.download_tools import format_date, text_extract, delete_hidden_sheets, month_to_number, month_abr_to_number, last_day_month
from models.download.Download_Base import Download_Base

class D_BO_000000032(Download_Base):
    ...
    def get_file_url(self, main_url, updated_to, format='%Y-%m-%d'):
        ...
        with sync_playwright() as pl:
            ...
            try:
                ...
                pasivas_button = page.query_selector('//a[contains(text(), "Pasivas")]')
                pasivas_button.click()
                print("Clicked on PASIVAS button.")

                # Expect and select "Activas del Sistema Financiero" from the dropdown list
                expect(page.get_by_text("Mensual"))
                print(pasivas_button.inner_html())
                select_dropdown_list = page.get_by_label("Tipo de Tasa Pasiva")
                select_dropdown_list.select_option("Caja de Ahorro")
                ...
                expect(page.locator('//td[@class="views-field views-field-field-archivo-tasa-pasiva-semxls views-align-left"]'))
                
                # Get all URLs of interest from the page
                files_urls = page.query_selector_all('//*[@id="quicktabs-tabpage-tab_tasas_de_interes-2"]//td')
                weekly_columns = [link.query_selector_all('//a') for link in files_urls[4:6]]
                index_searched = [index for index,value in enumerate(files_urls[4].inner_text().split("\n")) if re.search(r"\d{4}",value) and int(re.search(r"\d{4}",value).group()) >= updated_to.year]
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            if 'pdf' in extension: 
                ...
                date_line = [re.search(re_date,line.strip().lower()) for line in lines if re.search(re_date,line.strip().lower())]
                ...
                date = [re.search(re_date, str(year).strip().lower()) for year in sub_set if re.search(re_date, str(year).strip().lower())]
                date = date[-1].group()
...
Executor_D_BO_000000032 = D_BO_000000032
Robot = D_BO_000000032
