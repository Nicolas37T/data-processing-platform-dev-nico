...
class SNIS_BY_PARTS(Download_Base):    

    def verify_download(self, main_url,path,variable,filters_order=[],form_code='302',NUMBER_SPLIT = 3, COLUMN_TO_SPLIT=3, format='%Y-%m-%d'):
        """
            ...
            try:
                ...
                for link in gestion_links:
                    ...
                    for index,departamento_option_value in enumerate(departamento_options):
                        ...
                        semana_arrow_xpath_list = [f'//*[@id="ctl00_MainContent_ASPxPivotGrid1_ACCRowArea"]//td[@class="dxpgArea_Office2003Blue dxpgRowArea_Office2003Blue"][{COLUMN_TO_SPLIT}]//img[@class="dxPivotGrid_pgFilterButton_Office2003Blue dxpg__hfb"]',f'//*[@id="ctl00_MainContent_ASPxPivotGrid1_ACCRowArea"]//td[@class="dxpgArea_Office2003Blue dxpgRowArea_Office2003Blue"][{COLUMN_TO_SPLIT}]//img[@class="dxPivotGrid_pgFilterButtonActive_Office2003Blue dxpg__hfb"]']
                        
                        for i in range(NUMBER_SPLIT):
                            semana_arrow_xpath = semana_arrow_xpath_list[0] if i == 0 else semana_arrow_xpath_list[1]

                            new_page.wait_for_selector(semana_arrow_xpath, state='visible')
                            semana_arrow = new_page.query_selector(semana_arrow_xpath)
                            semana_arrow.click()
                            time.sleep(2)
                            new_page.wait_for_selector('//*[@id="ctl00_MainContent_ASPxPivotGrid1_DXHFP_PWC-1"]//table[@class="dxpgLoadingPanel_Office2003Blue"]', state='detached')
                            new_page.wait_for_selector('#ctl00_MainContent_ASPxPivotGrid1_TL', state='hidden')

                            inputs = new_page.query_selector_all('//input[@type="checkbox"]')
                                                       
                            
                            
                            all_input = inputs[0]
                            options_input = inputs[1:]
                        
                            chunk_size = (len(options_input) + NUMBER_SPLIT - 1) // NUMBER_SPLIT
                            options_input_batches = list(chunked(options_input, chunk_size))
                            selected_batch = options_input_batches[i]
                            print([o.get_attribute('name') for o in selected_batch])
                            all_input.set_checked(True)
                            all_input.set_checked(False)
                            [select.set_checked(True) for select in selected_batch]
                            time.sleep(3)
                            apply_button = new_page.query_selector('//*[@id="ctl00_MainContent_ASPxPivotGrid1_DXHFP_TPCFCm1_O"]')
                            apply_button.click()
                            time.sleep(2)
                            ...
                            path, f"{departamento_abr}_{variable['key_words']}{str('%02d' % (i,))}.{extension}")
                                download.save_as(download_path)
                                file_date = self.compare_files(file_path=download_path, year=int(year_downloaded))
                                new_path = os.path.join(
                            path,f"{file_date}_{departamento_abr}_{variable['key_words']}{str('%02d' % (i,))}.{extension}")
...
#     robot = SNIS_BY_PARTS()
#     x = robot.verify_download(main_url="https://estadisticas.minsalud.gob.bo/Reportes_Dinamicos/Menu_rep_dinamicos.aspx",updated_to="2022-12-21",path=r"\\10.0.0.9\spim\SNIS\Vigilancia Epidemiologica\04 Otras Infecciones")
#     print(x)