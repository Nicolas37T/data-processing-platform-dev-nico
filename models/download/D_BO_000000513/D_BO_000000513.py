...
class D_BO_000000513(BCB_Boletin_Mensual):

    def get_file_url(self, main_url, updated_to, key_words = "egreso divisas",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            for column in df_new_file.columns:
                years = df_new_file[column].astype(str).str.strip().str.extract(re_year,flags=re.IGNORECASE,expand=False).dropna()
                months = df_new_file[column].astype(str).str.strip().str.extract(re_month,flags=re.IGNORECASE,expand=False).dropna()
...
#     robot = Executor_D_BO_000000513()
    # x = robot.get_file_url(main_url="https://www.bcb.gob.bo/?q=pub_boletin-estadistico",updated_to="2023-12-31")
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"D:\DATAX\data-processing-platform\local_downloads\BCB\01-Informacion Estadistica\04-Sector Externo\01-Activos Internacionales\04-Egreso de Divisas\tmp\2024-08-21_16195908.05.xlsx"}]
    # y = robot.compare_files(files_paths=files,updated_to="2023-01-21")
    # print(y)

Executor_D_BO_000000513 = D_BO_000000513
Robot = D_BO_000000513
