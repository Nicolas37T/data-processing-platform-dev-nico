...
class D_BO_000000267(BCB_Sector_Externo):

    def get_file_url(self, main_url, updated_to, key_words = "servicio debido deuda acreedor",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            months = df.loc[header_index-1].astype(str).str.extract(re_month,expand=True,flags=re.IGNORECASE)[0].dropna()
            years = df.loc[header_index-2:header_index-1].stack().astype(str).str.extract(re_year,expand=True)[0].dropna()
...
Executor_D_BO_000000267 = D_BO_000000267
Robot = D_BO_000000267
