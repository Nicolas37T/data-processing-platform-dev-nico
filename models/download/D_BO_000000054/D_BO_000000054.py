...
class D_BO_000000054(Download_Base):

    def get_file_url(self, main_url, updated_to, format='%Y-%m-%d', key_words='compensa'):
        """
        ...
        with sync_playwright() as pl:
            ...
            try:
                ...
                for link in main_links:
                    ...
                    for url in urls:
                        match = re.search(re_month,url.replace('%20'," "),re.IGNORECASE).groups()
                        date_formated = pd.to_datetime(f"{match[0]}-{match[1]}-01",format=format) + pd.offsets.MonthEnd(0)
...
Executor_D_BO_000000054 = D_BO_000000054
Robot = D_BO_000000054
