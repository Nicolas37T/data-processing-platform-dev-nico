"""Type III download robot for BCB silver price quotations."""
...
class D_BO_000000577(Download_Base):
    """Download historical fine silver quotations from the BCB portal."""
    ...
    def check_new_data(
        ...
        """Build an Excel file with new silver data from the last seven days.
        ...
        try:
                ...
                records.append(
                    {
                        ...
                        (
                            "TIPO DE CAMBIO EN ONZA TROY FINA PLATA(S) "
                            "POR UNIDAD DE DOLAR (USD)"
                        ): cells[2],
                        (
                            "TIPO DE CAMBIO EN Bs POR UNIDAD DE "
                            "ONZA TROY FINA PLATA"
                        ): cells[3],
                    }
                )
            ...
            if latest_date <= pd.Timestamp(reference_date):
                return []

            base_name = unidecode(
                str(key_words or "Metales Preciosos Plata")
            )
            base_name = re.sub(r"[^A-Za-z0-9_-]+", "_", base_name)
            base_name = base_name.strip("_") or "metales_preciosos_plata"
            ...
            print(f"Could not download BCB silver quotations: {error}")
            return []


Robot = D_BO_000000577