            ...
            try:
                ...
                for attempt in range(1, max_retries + 1):
                    try:
                        ...
                        page.wait_for_selector('//*[@id="quicktabs-tabpage-tab_tasas_de_interes-0"]//table[contains(@class, "views-table")]/tbody/tr', state="visible", timeout=10 * 1000)
                        break
                    except Exception:
                ...
                stop = False
                while not stop:
                    files_rows = page.query_selector_all('//*[@id="quicktabs-tabpage-tab_tasas_de_interes-0"]//table[contains(@class, "views-table")]/tbody/tr')
                    if not files_rows:
                        break

                    for row in files_rows:
                        td1 = row.query_selector('td:nth-child(1), td')
                        if not td1:
                            continue
                        title = td1.inner_text()
                        ...
                        if match:
                            ...
                            if date_formated > updated_to:
                                urls_xls = row.query_selector_all('td:nth-child(3) a, td:nth-child(2) a')
                                urls_pdf = row.query_selector_all('td:nth-child(4) a, td:nth-child(3) a')
                                urls = urls_xls + urls_pdf
                                urls_hrefs = [u.get_attribute('href') for u in urls if u.get_attribute('href')]
                                files_urls.extend(urls_hrefs)
                            else:
                                stop = True
                                break
                    ...
                    next_btn = page.query_selector('//*[@id="quicktabs-tabpage-tab_tasas_de_interes-0"]//li[@class="pager-next"] a, .pager-next a')
                    if next_btn:
                        next_btn.click()
                        time.sleep(2)
            ...
            try:
                ...
                if 'pdf' in extension:
                    ...
                    for column in dataframe_to_look.columns:
                        for _, value in dataframe_to_look[column].items():
