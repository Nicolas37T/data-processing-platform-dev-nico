...
class D_BO_000000509(BCB_Boletin_Mensual):

    def get_file_url(self, main_url, updated_to, key_words = "exportacion zonas",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            df_new_file = df_new_file.dropna(how='all',axis=1).dropna(how='all')            
            
            years = df_new_file[df_new_file.columns[-2]].str.extract(re_year,flags=re.IGNORECASE,expand=False).dropna()
            months = df_new_file[df_new_file.columns[-2]].str.extract(re_month,flags=re.IGNORECASE,expand=False).dropna()
            
            year = int(years.iloc[-1])
            month = months.iloc[-1] if not months.empty else 'diciembre'
...
#     robot = Executor_D_BO_000000509()
    # x = robot.get_file_url(main_url="https://www.bcb.gob.bo/?q=pub_boletin-estadistico",updated_to="2023-12-31")
    # print(len(x))
    # print(x)

    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx','tmp_path': r"D:\DATAX\data-processing-platform\models\download\D_BO_000000509\2025-08-04_16061709.14.xlsx"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-12-31")
    # print(y)

Executor_D_BO_000000509 = D_BO_000000509
Robot = D_BO_000000509
