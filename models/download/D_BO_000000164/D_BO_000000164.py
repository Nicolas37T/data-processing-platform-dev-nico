...
import pandas as pd
from models.download.tools.download_tools import search_key_words, format_date,last_day_month, delete_hidden_sheets,  month_abr_to_number, month_to_number
from models.download.Download_Base import Download_Base

class D_BO_000000164(Download_Base):

    def get_file_url(self, main_url, updated_to, key_words = "indice remuneracion nominal actividad",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths: 
            ...
            df_new_file = pd.read_excel(file["tmp_path"],header=[7,8])
            ...
            years = [re.search(re_year, str(line)) for line in columns if re.search(re_year, str(line))]
            months = [re.search(re_month, str(value),re.IGNORECASE) for index,value in enumerate(columns) if re.search(re_month, str(value),re.IGNORECASE)]
...
#     print(len(x))
#     print(x)
#     # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"C:\Users\Kevin Padilla\Downloads\Bolivia - Indice de Salario Medio Nominal del Sector Privado segun Actividad Economica, 1996 - 2023.xlsx"}]
#     # y = robot.compare_files(files_paths=files,updated_to="2023-01-21")
#     # print(y)

Executor_D_BO_000000164 = D_BO_000000164
Robot = D_BO_000000164
