...
class D_BO_000000455(Download_Base):

    def get_file_url(self, main_url, updated_to, key_words = "oruro automotor",format='%Y-%m-%d'):
        """
        ...
        for file in files_paths: 
            ...
            if extension == '.zip':
                try:
                    ...
                    matched = [f for f in excel_files if search_key_words(text=_fix_zip_name(os.path.basename(f)), key_words='oruro segun')]
                    if not matched:
                        raise FileNotFoundError(f"No Excel file matching 'oruro segun' found in ZIP: {file['tmp_path']}")
...
Executor_D_BO_000000455 = D_BO_000000455
Robot = D_BO_000000455
