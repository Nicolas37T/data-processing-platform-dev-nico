...
import pandas as pd
from models.download.tools.download_tools import search_key_words, format_date, delete_hidden_sheets
from models.download.Download_Base import Download_Base

class D_BO_000000278(Download_Base):

    def get_file_url(self, main_url, updated_to, key_words = "bolivia pib departamento",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths: 
            ...
            df_new_file = pd.read_excel(file["tmp_path"], header=[9])
            for column in df_new_file.columns:
                if df_new_file[column].isna().all():
                    df_new_file.drop(columns=[column], inplace=True)
            
            years = [re.search(re_year, str(column)) for column in df_new_file.columns if re.search(re_year, str(column))]
...
#     robot = D_BO_000000278()
#     x = robot.get_file_url(main_url="https://www.ine.gob.bo/index.php/estadisticas-economicas/pib-y-cuentas-nacionales/producto-interno-bruto-departamental/producto-interno-bruto-departamental/#1589484093225-b57379da-b2f6",updated_to="2023-10-21")
#     print(len(x))
#     print(x)
...
Executor_D_BO_000000278 = D_BO_000000278
Robot = D_BO_000000278
