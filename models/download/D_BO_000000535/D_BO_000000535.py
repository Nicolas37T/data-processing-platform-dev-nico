...
class D_BO_000000535(CNDC):

    def get_file_url(self, main_url, updated_to, key_words="caudal semanal",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        re_date2 = r"(\d{4}\D\d{1,2}\D\d{1,2})"
        ...
        for file in files_paths:
            ...
            for column in df_new_file.columns:
                non_na_count = df_new_file[column].notna().sum()
                if df_new_file[column].isna().all() or non_na_count < len(df_new_file) / 2:
                    df_new_file.drop(columns=[column], inplace=True)              
                            
            date = df_new_file[df_new_file.columns[-1]].str.extract(re_date,flags=re.IGNORECASE,expand=True).dropna()
            if date.empty:
                date = df_new_file[df_new_file.columns[-1]].astype(str).str.extract(re_date2,flags=re.IGNORECASE).dropna().iloc[-1]                
...
#     robot = Executor_D_BO_000000535()
    # x =robot.get_file_url(main_url="https://www.cndc.bo/estadisticas/anual.php",updated_to='2024-08-01')
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"C:\Users\Kevin Padilla\Downloads\caudales_2024.zip"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-08-08")
    # print(y)


Executor_D_BO_000000535 = D_BO_000000535
Robot = D_BO_000000535
