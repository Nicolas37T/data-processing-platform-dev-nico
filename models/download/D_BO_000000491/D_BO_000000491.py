...
class D_BO_000000491(CNDC):
    ...
    def get_file_url(self, main_url, updated_to, format='%Y-%m-%d'):
        ...
        return self.get_file_url_main(main_url=main_url,updated_to=updated_to,key_words="volumen",frecuency='anual')
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            df_new_file = df_new_file.dropna()
            
            date = df_new_file[df_new_file.columns[0]].str.extract(re_date,flags=re.IGNORECASE,expand=True).dropna()
            if date.empty:
                date = df_new_file[df_new_file.columns[0]].astype(str).str.extract(re_date2,flags=re.IGNORECASE).dropna().iloc[-1]
...
#     robot = Executor_D_BO_000000491()
    # x =robot.get_file_url(main_url="https://www.cndc.bo/estadisticas/index.php",updated_to='2024-08-01')
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"D:\DATAX\data-processing-platform\models\download\D_BO_000000491\volumen_2024.zip"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-08-08")
    # print(y)


Executor_D_BO_000000491 = D_BO_000000491
Robot = D_BO_000000491
