...
class D_BO_000000098(BCB_Boletin_Mensual):

    def get_file_url(self, main_url, updated_to, key_words = "tasa libor",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            months = [re.search(re_month, str(value),re.IGNORECASE) for index,value in enumerate(df_new_file[df_new_file.columns[1]]) if re.search(re_month, str(value),re.IGNORECASE) and  pd.notna(df_new_file[df_new_file.columns[2]].iloc[index])]
...
Executor_D_BO_000000098 = D_BO_000000098
Robot = D_BO_000000098
