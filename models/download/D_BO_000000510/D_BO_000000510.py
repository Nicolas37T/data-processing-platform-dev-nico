...
class D_BO_000000510(BCB_Boletin_Mensual):

    def get_file_url(self, main_url, updated_to, key_words = "bienes destino",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            re_year = r'\b(\d{4})\s*[(]?p?[)]?$'
            re_month = r"-\s?\b((?:ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic))\b"
            ...
            months = []
            for column in df_new_file.columns:
                years_matches = df_new_file.loc[:10,column].astype(str).str.strip().str.extract(re_year,flags=re.IGNORECASE,expand=False).dropna()
                months_matches = df_new_file.loc[:10,column].astype(str).str.strip().str.extract(re_month,flags=re.IGNORECASE,expand=False).dropna()

                if not years_matches.empty:
                    years.extend(years_matches)
                if not months_matches.empty:
                    months.extend(months_matches)           
           
            year = int(years[-1])
            month = months[-1] if months else 'diciembre'
...
#     robot = Executor_D_BO_000000510()
    # x = robot.get_file_url(main_url="https://www.bcb.gob.bo/?q=pub_boletin-estadistico",updated_to="2023-12-31")
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"\\10.0.0.9\spim\BCB\01-Informacion Estadistica\04-Sector Externo\05-Importaciones\01-Bienes según uso o destino económico\tmp\2025-07-14_16071009.15.xlsx"}]
    # y = robot.compare_files(files_paths=files,updated_to="2023-01-21")
    # print(y)

Executor_D_BO_000000510 = D_BO_000000510
Robot = D_BO_000000510
