...
class D_BR_000000002(Download_Base):
    ...
    def get_file_url(self, main_url, updated_to, format='%Y-%m-%d'):
        ...
        links_xpath = '//div[@class="table-responsive"]//a'
        ...
        try:
            with sync_playwright() as pl:
                ...
                page.wait_for_selector(links_xpath,state='visible')
                
                link_searched = [item.get_attribute('href') for item in page.locator(links_xpath).all() if re.search(r'valor.+prazo',item.get_attribute('href'), re.IGNORECASE)]
                
                files_urls.extend(link_searched)
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        for file in files_paths:
            ...
            date = df_new_file[df_new_file.columns[0]].astype(str).str.extract(re_date).dropna()
            last_date = date.iloc[0].to_list()            
            last_date = format_date(f'{last_date[2]}-{last_date[1]}-{last_date[0]}')
...
Executor_D_BR_000000002 = D_BR_000000002
Robot = D_BR_000000002
