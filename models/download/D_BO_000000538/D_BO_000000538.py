...
class D_BO_000000538(CNDC):

    def get_file_url(self, main_url, updated_to, key_words="~max demanda 12 meses", format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        re_month_day = r"\b((?:enero|febrero|marzo|abril|mayo|junio|julio|agosto|septiembre|octubre|noviembre|diciembre|ene|feb|mar|abr|may|jun|jul|ago|sep|oct|nov|dic))\b\D*\b(\d{1,2})\b"
        re_year = r"\b(\d{4})\b"
        ...
        for file in files_paths:
            ...
            months_days = df_new_file[df_new_file.columns[-1]].str.extract(re_month_day,flags=re.IGNORECASE,expand=True).dropna()
            month_index = months_days.index[0]
            years = df_new_file.loc[month_index-1].astype(str).str.extract(re_year,flags=re.IGNORECASE,expand=True).dropna()

            year = int(years.iloc[-1].item())
            month = months_days.iloc[0][0]
            month = month_to_number(month) if month_to_number(month) else month_abr_to_number(month)
            day = months_days.iloc[0][1]
...
#     robot = Executor_D_BO_000000538()
    # x =robot.get_file_url(main_url="https://www.cndc.bo/estadisticas/index.php",updated_to='2024-10-01')
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"C:\Users\Kevin Padilla\Downloads\demcons20241130.zip"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-08-09")
    # print(y)


Executor_D_BO_000000538 = D_BO_000000538
Robot = D_BO_000000538
