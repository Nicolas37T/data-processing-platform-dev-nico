...
class D_BO_000000500(CNDC):

    def get_file_url(self, main_url, updated_to, key_words="balance",format='%Y-%m-%d'):
        """
    ...
    def compare_files(self, files_paths, updated_to, format='%Y-%m-%d'):
        ...
        re_date2 = r"(\d{1,2}\D?\d{1,2}\D?\d{2})"
        ...
        for file in files_paths:
            ...
            col = df_new_file[df_new_file.columns[0]].astype(str)

            parsed = pd.to_datetime(col, errors="coerce").dropna()

            if not parsed.empty:
                date_formated = parsed.iloc[0]
                ...
                date = col.str.extract(re_date2, flags=re.IGNORECASE).dropna().iloc[0]
                date_formated = format_date(date[0],format="%d/%m/%y")
...
#     robot = Executor_D_BO_000000500()
    # x =robot.get_file_url(main_url="https://www.cndc.bo/estadisticas/index.php",updated_to='2024-08-09')
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://www.bcb.gob.bo/webdocs/publicacionesbcb/2024/04/10/01.01P.xlsx', 'tmp_path': r"D:\DATAX\data-processing-platform\models\download\D_BO_000000500\bal_140824.zip"}]
    # y = robot.compare_files(files_paths=files,updated_to="2024-08-09")
    # print(y)


Executor_D_BO_000000500 = D_BO_000000500
Robot = D_BO_000000500
