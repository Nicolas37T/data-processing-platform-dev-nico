import re, traceback, os, zipfile, unicodedata
...
class D_BO_000000454(Download_Base):

    def get_file_url(self, main_url, updated_to, key_words = "cochabamba automotor",format='%Y-%m-%d'):
        """
        ...
        for file in files_paths: 
            ...
            if extension == '.zip':
                try:
                    ...
                    matched = [f for f in excel_files if search_key_words(text=_fix_zip_name(os.path.basename(f)), key_words='cochabamba segun')]
                    if not matched:
                        raise FileNotFoundError(f"No Excel file matching 'cochabamba segun' found in ZIP: {file['tmp_path']}")
            ...
            df_new_file = df_new_file.dropna(how='all').dropna(axis=1,how='all')

            num_mask = df_new_file.map(lambda x: pd.to_numeric(x,errors='coerce')).notna().any(axis=1)
            
            headers_index = df_new_file.loc[num_mask].index       
            
            years = [m for line in df_new_file.loc[headers_index[0]] if (m:=re.search(re_year, str(line)))]
...
#     robot = Executor_D_BO_000000454()
    ...
    # print(len(x))
    # print(x)
    # files = [{'download_url': 'https://nube.ine.gob.bo/index.php/s/PnEqp7IopHEJDBu/download', 'tmp_path': r'D:\DATAX\data-processing-platform\models\download\D_BO_000000454\2025-08-14_200240download.xlsx'}]
    # y = robot.compare_files(files_paths=files,updated_to="2022-01-21")
    # print(y)

Executor_D_BO_000000454 = D_BO_000000454
Robot = D_BO_000000454
