...
class D_BO_000000447(Download_Base):
    ...
    def get_file_url(self, main_url, updated_to,format='%Y-%m-%d'):
        ...
        try:
            ...
            # Iterate over years from updated_to year to the current year
            for year in range(updated_to.year,today.year+1):
                # Construct the URL to request files for the current year
                url = f"https://www.aps.gob.bo/?option=com_ajax&module=apsfilesdisplayer&method=getFiles&format=json&folder=webdocs___UNE___DS___Estadisticas___Boletin+Estadistico+Mensual+de+Seguros___{year}&format=json"
                ...
                for file in files:
                    ...
                    # Check if the formatted date is after updated_to
...
Executor_D_BO_000000447 = D_BO_000000447
Robot = D_BO_000000447
