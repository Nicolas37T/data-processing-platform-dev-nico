"""Migration robot for report D_BO_000000485_01: Cotización del Oro (SENARECOM)."""
...
class D_BO_000000485_01(Migration_Base):
    """Migration robot for D_BO_000000485_01."""

    def standard_report(self, dataframe: pd.DataFrame, conversion_factor: int) -> Tuple[dict, pd.DataFrame]:
        """
        Enrich dataframe with metric and metric unit metadata based on nv1.
        nv1 contains column header indicating currency (USD or BOB per Ounce/Troy):
        - metrica = 'moneda'
        - unidad_metrica = 'USD' o 'BOB' (o 'USD/OT', 'BOB/OT')
        - conversion_factor = 1
        """
        # Clean column names
        ...
        def get_unit(val: str) -> str:
            v = str(val).upper()
            if "DOLAR" in v or "USD" in v or "$" in v:
                return "USD"
