"""Migration robot for report D_BO_000000419_02: Información monetaria (saldos en millones de Bs) (BCB)."""
...
class D_BO_000000419_02(Migration_Base):
    """Migration robot for D_BO_000000419_02."""
    ...
    def standard_report(self, dataframe: pd.DataFrame, conversion_factor: int) -> Tuple[dict, pd.DataFrame]:
        ...
        Saldos en millones de BOB:
        - metrica = 'moneda'
        - unidad_metrica = 'BOB'
        - factor = 1,000,000 (para convertir millones a BOB completos)
        """
        # Clean column names
        dataframe.columns = [re.sub(r"[\t\xa0]+", "", str(c)).strip() for c in dataframe.columns]

        # Standardize strings in hierarchy columns
        for col in ["titulo1", "nv1", "nv2", "nv3", "nv4", "nv5", "nv6"]:
